...
To add: semivariogram (used to check for the right residual correlation structure)
...
# Mean BPRS by age (continuous gradient)
ggplot(dat_long, aes(x = time, y = bprs, color = age, group=age)) + 
  stat_summary(fun = mean, geom = "line", size = 1) +
  scale_color_viridis_c(name = "Age") +
  theme_minimal() +
  labs(
    title = 'Average BPRS Score as Function of Time and Age',
    x = 'Year',
    y = 'BPRS Score'
  )
...
This pattern of dropout explains why the overall variance of BPRS appears constant over time. Although sample size decreases, the reduction in the range of BPRS values among remaining participants likely counterbalances the expected increase in variance.

Based on these findings, a linear mixed-effects model is the most appropriate statistical approach for analyzing this longitudinal dataset. Time and age should be included as fixed effects, with patient ID modeled as a random effect to account for repeated measures within individuals. Since patients are also nested within trials, trial can be included as an additional random effect. The exploratory analysis suggests that random intercepts are necessary, as individuals differ substantially in baseline BPRS levels. However, the approximately parallel trajectories across time indicate that random slopes may not be required, though this will be verified through formal hypothesis testing.

Finally, given that the data are collected at regular time intervals, it is reasonable to assume an autoregressive correlation structure for the residuals, capturing the expected correlation between adjacent time points.
