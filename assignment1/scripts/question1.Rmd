...
```{r}
############################################################
# PACKAGES REQUIRED FOR QUESTION 1
############################################################
...
libraries_q1 <- c(
  ...
  "ggplot2",    # plots
  "reshape2",   # melt()
  "viridis",    # viridis color scales
  "knitr",      # knitr tables
  "kableExtra"  # styled tables
)

# Install missing packages
for (pkg in libraries_q1) {
...
# Load packages
invisible(lapply(libraries_q1, library, character.only = TRUE))


############################################################
# LOAD RAW DATA (ALWAYS FROM SCRATCH)
############################################################
...
############################################################
# CANONICAL DATA PRE-PROCESSING FOR ALL QUESTIONS
# (Q2–Q5 MUST FOLLOW THE SAME RULES)
############################################################
...
to_factor <- function(dat_long) {
  ...
  dat_long <- dat_long %>%
    mutate(
      ...
      job      = as.factor(job),
      wzc      = as.factor(wzc),
      time_num = as.numeric(time) - 1    # numeric 0–6
    )
...
continuous_vars <- c(
  "age", "bmi", "inkomen", "adl",
  "bprs0", "cdrsb0", "abpet0", "taupet0"
)

baseline_continuous <- dat %>%
...
categorical_vars <- c("sex", "edu", "job", "wzc")

baseline_categorical <- lapply(categorical_vars, function(v) {
  
  df <- dat %>%
    count(!!sym(v), name = "Frequency") %>%
    mutate(
...
for (v in unique(mean_var_summary$variable)) {
  
  df_v <- mean_var_summary %>% filter(variable == v)
  
  p <- ggplot(df_v, aes(x = year, y = mean_value)) +
    ...
    labs(
      title = paste("Mean ± SD across years –", v),
      x = "Year", y = "Mean ± SD"
    ) +
...
# Convert to long format, keeping age_bin
dat_long2 <- dat2 %>%
  pivot_longer(
    cols = starts_with("bprs"),
    names_to = "measurement_occasion",
    values_to = "bprs_value"
  ) %>%
  mutate(
    year = as.numeric(gsub("bprs", "", measurement_occasion))
  )

# Count non-missing observations by year × age_bin
plot_data <- dat_long2 %>%
  filter(!is.na(bprs_value)) %>%
...
ggplot(plot_data, aes(x = factor(year), y = count, fill = age_bin)) +
