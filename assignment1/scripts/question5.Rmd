...
m5 <- lmer(bprs ~ time_num * (sex + age + wzc + adl + job) + taupet0 + abpet0 + cdrsb0 + (time_num | trial/patid), 
           data = dat_long, REML = TRUE) # model with random intercept and slope for
# patid and trial
...
Both taupet0 and abpet0 are not significant. We might choose to remove them from the model
```{r}
m7 <- lmer(bprs ~ time_num * (age + adl + job + cdrsb0) + sex  + wzc + (time_num | trial/patid)
           ,data = dat_long, REML = FALSE)
summary(m7)

anova(m7, m6)
```
The more complex model's fit is not significantly better. We can then choose to continue with the simpler model, m7. 



```{r}
m7_REML <- lmer(bprs ~ time_num * (age + adl + job + cdrsb0) + sex  + wzc + (time_num | trial/patid)
...
```{r}
re <- ranef(m7_REML, condVar = TRUE)

# QQ plot of the intercepts
qqnorm(ranef(m7_REML)$`patid:trial`[, "(Intercept)"])
qqline(ranef(m7_REML)$`patid:trial`[, "(Intercept)"])

```
```{r}
# QQ plot of the slopes
qqnorm(ranef(m7_REML)$`patid:trial`[, "time_num"])
qqline(ranef(m7_REML)$`patid:trial`[, "time_num"])
