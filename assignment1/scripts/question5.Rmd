...
m6 <- lmer(bprs ~ time_num * (age + adl + job + wzc) + sex  + taupet0 + abpet0 + cdrsb0 + (time_num | trial/patid)
           ,data = dat_long, REML = FALSE)
summary(m6)

anova(m5, m6)

m7 <- lmer(bprs ~ time_num * (age + adl + job) + sex + wzc + taupet0 + abpet0 + cdrsb0 + (time_num | trial/patid), data = dat_long, REML = FALSE)
summary(m7)
